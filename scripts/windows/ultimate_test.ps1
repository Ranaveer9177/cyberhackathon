...
if ($Help) {
    Write-Host "VibeGuard Ultimate Test v6.0.0"
...
$verProc.WaitForExit()
$verOut = if (Test-Path $tmpVer) { (Get-Content $tmpVer).Trim() } else { "VibeGuard v6.0.0" }
$verStr = if ($verOut -match 'v\d+\.\d+\.\d+') { $matches[0] } else { "v6.0.0" }
