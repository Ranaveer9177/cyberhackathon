...
VibeGuard v4.7.0 installs as a native, self-locating global CLI utility on Windows. Running `scripts\windows\setup.bat` configures the tool into `%LOCALAPPDATA%\VibeGuard` and idempotently registers the directory into the Current User's `PATH`.
